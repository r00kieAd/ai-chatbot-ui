...
export interface ShuffleProps {
  ...
  /**
   * When true, start the animation immediately on mount (no ScrollTrigger gating).
   * Keeps the component visible without relying on scroll position.
   */
  autoplay?: boolean;
  /**
   * When true, force visibility via inline style to avoid brief hidden state.
   */
...
const Shuffle: React.FC<ShuffleProps> = ({
  ...
  useGSAP(
    () => {
      ...
      const play = () => {
        ...
        const tl = gsap.timeline({
          ...
          onRepeat: () => {
            ...
            gsap.set(strips, { x: (i, t: HTMLElement) => { void i; return parseFloat(t.getAttribute('data-start-x') || '0'); } });
            onShuffleComplete?.();
          },
          onComplete: () => {
            playingRef.current = false;
      ...
      // If autoplay, bypass ScrollTrigger gating and run immediately
      ...
      return () => {
        ...
        removeHover();
        teardown();
        // Do not hide on cleanup; keep it visible if it was shown
      };
    },
    {
      dependencies: [
  ...
  const commonStyle: React.CSSProperties = { textAlign, ...style, visibility: (forceVisible || ready) ? 'visible' : undefined };
